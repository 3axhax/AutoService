    ...
    <>
      ...
      {tableData.rows.length > 0 ? (
        tableData.rows.map((row: TableDataRow[], index: number) => (
          <div
            key={index}
            className={`flex flex-col lg:flex-row mb-4 lg:mb-0 rounded-lg lg:rounded-none border-1 lg:border-t-0 border-blue-900 bg-white hover:bg-blue-light lg:odd:bg-stone-200/50 last:rounded-b-lg ${className ? ` ${className}` : ""}`}
          >
            {row.map((cell: TableDataRow) => (
              <div
                key={cell.name}
                className={`flex-1 px-3 py-1 lg:py-2 text-lg lg:text-base text-gray-900 lg:border-r-1 last:border-r-0 border-blue-900 ${cell.className ? ` ${cell.className}` : ``}`}
              >
                {cell.label ? (
